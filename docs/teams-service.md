...
| `POST /snapshots` | `{snapshot}` stores a capsule snapshot of references and returns `{hash, author, createdAt, snapshot}`. The hash is SHA-256 over the canonical snapshot; the same snapshot stores once (200 instead of 201). |
| `GET /snapshots/:hash` | Returns a stored snapshot to any active member. |
| `GET /threads/:task` | Returns `{thread, entries}`: the thread summary and every entry, unordered; clients order by Lamport clock, author, sequence. |
| `POST /threads/:task/entries` | Appends one entry: `{kind, text?, place?, snapshot?, target?, to?, parents?, prev?, lamport, title?, idempotencyKey}`. Creates the thread on first append. Returns `{entry, thread}`. |
| `POST /together` | `{active, task?, pointer?, editor?, leading?, following?}` publishes the caller's ephemeral cursor; `{active: false}` withdraws it. Never persisted. |
...
`GET /state` returns the lanes, proposals and assets described below, plus `threads` (summaries), `inbox` (hand-offs addressed to the caller that they have not answered) and `together` (the cursors currently shared in the room):
...
## Threads, capsules and Together

These routes carry the Rabta Teams way of working described in `rabta-teams-plan.md`. Everything stored is a reference, never a file's contents.

```ts
type Snapshot = {
  title: string; project: { id: string; name: string };
  files: string[]; links: string[]; folders: string[]; pins: string[];
  branch: string | null; activeFile: string | null; note: string;
};
type Place =
...
type Entry = {
  hash: string; seq: number; task: string; author: string; lamport: number;
  kind: "knot" | "request" | "decision" | "acknowledge" | "handoff" | "accept" | "decline" | "split" | "weave" | "link";
  text: string; place: Place | null; snapshot: string | null; target: string | null;
  to: string | null; parents: string[]; prev: string | null; createdAt: string;
};
type Cursor = {
  memberId: string; task: string | null; pointer: { x: number; y: number } | null;
  editor: { path: string; line: number; column: number; selection?: { line: number; column: number } } | null;
  leading: boolean; following: string | null; at: string;
};
```

- **Snapshots** are content-addressed. Every file, folder and `activeFile` reference must be project-relative or `~/`-relative; absolute paths, parent segments and control characters are rejected with 400. Links are plain `http(s)` without credentials, query or fragment. Limits: 200 files, 100 links, 50 folders, 100 pins, 1,024 characters per reference, 160 for the title, 2,000 for the note, 5,000 snapshots per room.
- **Threads** are append-only and keyed by a task id matching `^[A-Za-z0-9._-]{1,80}# Rabta Teams service
...
. Each entry's `hash` is SHA-256 over its canonical stored fields, and `prev` lets a client name the entry it saw last. The service sets `lamport` to one more than the larger of the thread's clock and the caller's, so clients order deterministically by `(lamport, author, seq)`. Entries are validated by kind: knots, requests and decisions need text (5,000 characters at most); a hand-off needs a stored snapshot, a next step and an active recipient other than the author; `accept` and `decline` may only come from that recipient, once; `acknowledge` applies to decisions, once per member; `weave` names one or two existing entries and carries a snapshot; `link` points at a link or commit place. Limits: 5,000 entries per thread, 2,000 threads per room. Repeating an append with the same idempotency key and payload returns the existing entry, across restarts.
- **Together** is ephemeral. Cursor updates are held in memory for 60 seconds after the last update and are never written to the store. Pointer coordinates are fractions of the stage in 0..1; editor paths are project-relative. Each member has its own budget of 15 cursor updates per second, separate from the request limit. Revoking a member removes their cursor at once.
...
- SSE sends `ready` with `{revision}`, `change` with `{revision}`, `presence` with `{memberId,status,lastSeenAt}`, `revoked` with `{}`, `thread` with `{task, seq, kind, author, hash}` after a committed append, and `cursor` with the full cursor or `{memberId, gone: true}` when one is withdrawn or expires. Heartbeat comments arrive every 15 seconds. Events contain no draft contents or credentials. Refresh `/state` after `ready` or `change`; reconnect always obtains the current state, so missing historical events cannot lose saved work.
...
Run `node --test services/teams/` from the repository root (26 tests: the HTTP/SSE suite and the pure thread rules). The suite creates real HTTP servers and independent client credentials against temporary durable directories. It verifies private lane isolation, concurrent same-lane conflicts, published previews and withdrawal, proposal ownership and review/application races, idempotent sending, room isolation, single-use/revoked invitations, lost-response create/join recovery, live presence and revocation, asset upload/download/deletion permissions and size limits, restart/reconnect, origin and request limits, single-writer locking, corrupt-state recovery, content-addressed snapshots with reference validation, thread appends with idempotency across a restart, the inbox, Together broadcast, expiry and revocation, and the separate cursor rate budget.
