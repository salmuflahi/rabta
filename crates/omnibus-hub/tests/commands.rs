...
async fn hub_with_echo_connector(cfg: HubConfig) -> (Hub, String, tempfile::TempDir) {
    ...
    let id = welcome["payload"]["connectorId"]
        .as_str()
        .unwrap()
        .to_string();
...
async fn routes_command_and_correlates_response() {
    let dir = tempfile::tempdir().unwrap();
    let (hub, id, _dir) = hub_with_echo_connector(HubConfig::new(dir.path().to_path_buf())).await;
    let result = hub
        .send_command(&id, "workspace.open", json!({"path": "/tmp/x"}))
        .await
        .unwrap();
...
async fn command_times_out_when_connector_never_answers() {
    ...
    let err = hub
        .send_command(&id, "workspace.open", json!({}))
        .await
        .unwrap_err();
    assert!(matches!(err, CommandError::Timeout));
    assert!(
        started.elapsed() < Duration::from_secs(2),
        "timeout must respect config"
    );
}
