...
async fn hello_with(port: u16, extra: Value) -> Value {
    ...
    ws.send(
        json!({"v":1,"id":"h","kind":"hello","payload":payload})
            .to_string()
            .into(),
    )
    .await
...
async fn token_auth_uses_live_map_keyed_by_name_and_kind() {
    ...
    cfg.tokens
        .write()
        .unwrap()
        .insert("t/fake".into(), "tok-1".into());
    let hub = Hub::start(cfg).await.unwrap();
    assert_eq!(
        hello_with(hub.port(), json!({"token": "tok-1"})).await["kind"],
        "welcome"
    );
    assert_eq!(
...
async fn pairing_approve_issues_working_token() {
    ...
    assert!(
        hub.resolve_pairing(&pairing_id, Some("tok-new".into()))
            .await
    );
...
async fn pairing_deny_and_timeout() {
    ...
    let (mut ws, _) = tokio_tungstenite::connect_async(format!("ws://127.0.0.1:{}", hub.port()))
        ...
        .unwrap();
    ws.send(
        json!({"v":1,"id":"p","kind":"pair","payload":{"name":"a","kind":"chrome"}})
            .to_string()
            .into(),
    )
    .await
    ...
    // Timeout.
    let (mut ws2, _) = tokio_tungstenite::connect_async(format!("ws://127.0.0.1:{}", hub.port()))
        .await
        .unwrap();
    ws2.send(
        json!({"v":1,"id":"p","kind":"pair","payload":{"name":"b","kind":"chrome"}})
            .to_string()
            .into(),
    )
    .await
...
async fn duplicate_pairing_request_is_rejected() {
    ...
    let (mut ws1, _) = tokio_tungstenite::connect_async(format!("ws://127.0.0.1:{}", hub.port()))
        .await
        .unwrap();
    ws1.send(
    ...
    assert!(
        !saw_second,
        "a second PairingRequested was broadcast for a duplicate pair request"
    );
...
async fn pair_frame_with_mismatched_envelope_version_is_version_mismatch() {
    ...
    assert!(
        !saw_it,
        "PairingRequested was emitted despite envelope version mismatch"
    );
    assert!(hub.pending_pairings().await.is_empty());
}
...
async fn invalid_bytes_origin_header_is_rejected() {
    ...
    let mut req = format!("ws://127.0.0.1:{}", hub.port())
        ...
        .unwrap();
    req.headers_mut()
        .insert("Origin", HeaderValue::from_bytes(&[0xff, 0xfe]).unwrap());
    let attempt = tokio_tungstenite::connect_async(req).await;
    assert!(
        attempt.is_err(),
        "origin header with invalid bytes must be rejected, not fail open"
    );
}
